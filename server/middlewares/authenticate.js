  ...
  try {
    ...
    if (err == "Error: No token provided") {
      ...
      res
        .status(400)
        .send(
          `<h3>No token provided: Please login to access the page. <br> <a href="/login">Login</a></h3>`
        );
    } else if ((err = "Error: Token could not be verified")) {
      res.status(400).render("login.ejs", { error: "Please login to access" });
      // .send("Token could not be verified: Incorrect token provided");
    } else {
      res
        .status(400)
        .send(`Unauthorized: Something went wrong. Please relogin`);
    }
  }
};
