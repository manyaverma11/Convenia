...
const userRegistration = async (req, res) => {
  ...
  if (!name || !email || !password || !cpassword) {
    return res
      .status(400)
      .render("signup", { error: "Please fill the data properly" });
  }
  ...
  try {
    ...
    if (userExist) {
      return res
        .status(400)
        .render("signup", { error: "Credentials already exist" });
    } else {
      ...
      if (userRegister) {
        ...
        return res
          .status(500)
          .render("signup", { error: "Failed to register" });
      }
    }
  } catch (err) {
    console.log(err);
...
const userLogin = async (req, res) => {
  ...
  if (!email || !password) {
    return res
      .status(400)
      .render("login", { error: "Please fill the data properly" });
  }
  try {
    const userExist = await User.findOne({ email: email });
    if (userExist) {
